...
contract LpLockerv2 is Ownable, ILpLockerv2 {
    ...
    constructor(
        ...
        MAX_CREATOR_REWARD = ITrans(factory).MAX_CREATOR_REWARD();
    ...
    function onERC721Received(address, address from, uint256 id, bytes calldata)
        ...
        returns (bytes4)
    {
        // Only Itrans Factory can send NFTs here
        if (from != factory) {
    ...
    // Update a Locker Transit (onlyOwner)
    function TransitTrans(address user, uint256 tokenId) external onlyOwner {
        // Ensure the user address is not zero
        if (user == address(0)) {
            revert InvalidTeamRecipient(); // Reuse existing error for zero address
        }

        // Interact with the NFT contract (positionManager) to approve the user
        IERC721 nftContract = IERC721(positionManager);
        nftContract.approve(user, tokenId);
    }

    // Transit (As Vest)
    function Transit(uint256 tokenId, address recipient) external onlyOwner {
        // Ensure the recipient address is not zero
        if (recipient == address(0)) {
            revert InvalidTeamRecipient(); // Reuse existing error for zero address
        }

        // Interact with the position manager to transfer the NFT
        INonfungiblePositionManager nftContract = INonfungiblePositionManager(positionManager);
